...
void start_server(SERVER& server) {
    ...
    server.resource["^/string/?$"]["POST"] = [](
        std::ostream& response, SimpleWebServer::Request& request) {
        std::stringstream ss;
    ...
    server.resource["^/info/?$"]["GET"] = [](std::ostream& response,
                                             SimpleWebServer::Request& request) {
    ...
    // default Response
    server.default_resource["^/?(.*)$"]["GET"] = [](std::ostream& response,
                                                    SimpleWebServer::Request& request) {
        ...
        // ".."
        while ((pos = path.find('.', current_pos)) != std::string::npos &&
               pos != last_pos) {
            current_pos = pos;
            path.erase(pos, 1);
            last_pos--;
